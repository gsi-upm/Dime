...
public class DimeOutlineTreeProvider extends DefaultOutlineTreeProvider {
	...
	protected void _createChildren(IOutlineNode parentNode, EObject modelElement) {
		...
		else if(modelElement.eClass().getName().equals("Send")){
			send_createChildren(parentNode, (Send) modelElement);
			
		}
		else if(modelElement.eClass().getName().equals("State")){
			 state_createChildren(parentNode, (State) modelElement);
		}
		else if(modelElement.eClass().getName().equals("Block")){
			 block_createChildren(parentNode, (Block) modelElement);
		}
	...
	public void block_createChildren(IOutlineNode parent, Block m) {
		for(AbstractElement e: m.getSta()){
			createNode(parent,e);
		}
