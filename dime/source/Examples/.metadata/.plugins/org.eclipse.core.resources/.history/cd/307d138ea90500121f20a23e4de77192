...
function app_start() {
	...
	$tropo->on(array("event" => "continue", "next" =>"record.php?uri=end"));
...
function app_end() {
	...
	$tropo->say("ftp://totemteleko:telcodev@ftp.tropo.com/recordings/record_".$_REQUEST['lastState_dime'].".mp3", array("allowSignals"=>array("timeExceeded", "attemptsLimit")));
	$tropo->hangup();
